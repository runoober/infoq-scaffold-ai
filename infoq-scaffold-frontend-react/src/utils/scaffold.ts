...
export const handleTree = <T>(data: any[], id?: string, parentId?: string, children?: string): T[] => {
  ...
  const tree: T[] = [];
  for (const d of data) {
    const currentId = d[config.id];
    childrenListMap[currentId] = d;
  }

  for (const d of data) {
    const currentParentId = d[config.parentId];
    const parentObj = childrenListMap[currentParentId];
    if (!parentObj || parentObj === d) {
      tree.push(d);
    } else {
      if (!Array.isArray(parentObj[config.childrenList])) {
        parentObj[config.childrenList] = [];
      }
      if (!parentObj[config.childrenList].some((child: any) => child && child[config.id] === d[config.id])) {
        parentObj[config.childrenList].push(d);
      }
    }
  }

  const trimEmptyChildren = (nodes: any[]) => {
    for (const node of nodes) {
      const childList = node[config.childrenList];
      if (Array.isArray(childList)) {
        const validChildren = childList.filter(
          (child) => child && child[config.id] !== undefined && child[config.id] !== null && child[config.id] !== ''
        );
