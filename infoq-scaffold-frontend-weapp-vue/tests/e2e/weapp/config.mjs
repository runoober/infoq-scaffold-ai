...
export function loadRuntimeConfig({ suite, report }) {
  ...
  const distDir = resolveDistDir({
    workspaceRoot,
    explicitPath: String(process.env.WEAPP_E2E_PROJECT_PATH || '').trim()
  });
...
function resolveDistDir({ workspaceRoot, explicitPath }) {
  const candidates = [];
  if (explicitPath) {
    candidates.push(explicitPath);
  }
  candidates.push('dist', 'dist/build/mp-weixin', 'dist/dev/mp-weixin', 'unpackage/dist/build/mp-weixin', 'unpackage/dist/dev/mp-weixin');

  const resolved = candidates
    .map((entry) => path.resolve(workspaceRoot, entry))
    .find((candidate) => existsSync(path.join(candidate, 'project.config.json')));

  if (resolved) {
    return resolved;
  }

  return path.resolve(workspaceRoot, explicitPath || 'dist');
}
...
function parseDistCompileEnv(distDir) {
  const candidateFiles = [
    ...
    path.join(distDir, 'common', 'vendor.js')
