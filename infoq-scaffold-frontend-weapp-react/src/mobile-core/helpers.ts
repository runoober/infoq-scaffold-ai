...
import type { DictDataVO, DictOption, TableResponse } from './types';
...
export const asCaptchaImage = async (img?: string, cacheKey?: string) => {
  ...
  if (userDataPath && taroApi.base64ToArrayBuffer && taroApi.getFileSystemManager) {
    try {
      ...
      console.warn('[infoq-mobile-core] Failed to materialize captcha image for weapp, fallback to data URL.', error);
    }
  }
  return dataUrl;
};
