...
插件治理的详细矩阵见 [`plugin-catalog.md`](/collaboration/plugin-catalog)。
...
- 快速启动：[`quick-start.md`](/guide/quick-start)
- 部署前准备：[`deploy-prerequisites.md`](/devops/deploy-prerequisites)
- Compose 部署：[`docker-compose-deploy.md`](/devops/docker-compose-deploy)
- 插件治理：[`plugin-catalog.md`](/collaboration/plugin-catalog)
