...
在 `infoq-scaffold-ai` 中，`AGENTS.md` 的目标不是“写很多解释”，而是用尽量短的索引行，把高优先级规则稳定放进上下文。当前仓库规则采用“中文说明为主，英文命令/路径/标识符保留原文”的写法。
...
因此，`AGENTS.md` 只保留必须常驻上下文的内容，解释性材料放到文档目录或 skill 内部。语言可以是中文，但命令、路径、文件名、skill 名和环境变量这类精确标识不应翻译。
...
- `AGENTS.md` 可以中文化，但要继续保持短、准、可执行
- `|IMPORTANT:`、命令、路径、skill 名、文件名、env 名这类精确标识保持原文更稳
