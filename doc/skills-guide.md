...
`.agents/skills` 是本仓库的能力库。它的核心目标不是存放提示词，而是把可重复的研发动作沉淀成可执行 SOP，让 Codex 在真实仓库里稳定复用。
...
这意味着 skill 的关键不只是脚本，还包括触发描述写得是否准确。当前仓库允许用中文描述触发语义，但 `skill` 名称、路径、命令和产物文件名仍保持英文原文。
...
| `infoq-vue-browser-automation` | Vue 项目专用浏览器验证 | Vue 本地联调、路由校验、登录态注入 |
| `infoq-react-browser-automation` | React 项目专用浏览器验证 | React 本地联调、路由校验、登录态注入 |
| `infoq-vue-run-dev-stack` | 启动/停止 Vue 本地联调环境 | 后端 + Vue、本地 Vue 联调 |
| `infoq-react-run-dev-stack` | 启动/停止 React 本地联调环境 | 后端 + React、本地 React 联调 |
...
- Vue 本地联调不再靠口头说明，而是交给 `infoq-vue-run-dev-stack`
- React 本地联调不再靠口头说明，而是交给 `infoq-react-run-dev-stack`
...
- 中文触发描述尽量贴近真实用户说法
- 中英文混写时，优先让可执行标识保持精确，不要为了“全中文”牺牲可路由性
...
- 版本升级 skill：`../.agents/skills/infoq-version-bump/SKILL.md`
