...
public class DataPermissionPointcut extends StaticMethodMatcherPointcut {
    ...
    public boolean matches(Method method, Class<?> targetClass) {
        ...
        if (Proxy.isProxyClass(targetClassRef)) {
            ...
            Class<?>[] interfaces = targetClassRef.getInterfaces();
            if (interfaces.length == 0) {
                log.warn("data permission pointcut skips proxy without interfaces: {}", targetClassRef.getName());
                return false;
            }
            targetClassRef = interfaces[0];
