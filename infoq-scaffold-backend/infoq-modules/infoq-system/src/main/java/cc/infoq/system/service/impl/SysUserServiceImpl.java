...
public class SysUserServiceImpl implements SysUserService, UserService {
    ...
    public List<SysUserExportVo> selectUserExportList(SysUserBo user) {
        ...
        if (ObjectUtil.isNotNull(user.getDeptId())) {
            deptIds = sysDeptMapper.selectDeptAndChildById(user.getDeptId());
        }
        return sysUserMapper.selectUserExportList(user, deptIds, Wrappers.query());
    }
    ...
    public TableDataInfo<SysUserVo> selectAllocatedList(SysUserBo user, PageQuery pageQuery) {
        ...
        Page<SysUserVo> page = sysUserMapper.selectAllocatedList(pageQuery.build(), wrapper);
        return TableDataInfo.build(page);
    }
    ...
    public TableDataInfo<SysUserVo> selectUnallocatedList(SysUserBo user, PageQuery pageQuery) {
        ...
        wrapper.eq("u.del_flag", SystemConstants.NORMAL)
            ...
            .like(StringUtils.isNotBlank(user.getPhonenumber()), "u.phonenumber", user.getPhonenumber())
            .orderByAsc("u.user_id");
        Page<SysUserVo> page = sysUserMapper.selectUnallocatedList(pageQuery.build(), wrapper);
        return TableDataInfo.build(page);
    }
    ...
    private void fillUserDeptName(SysUserVo user) {
        if (ObjectUtil.isNull(user) || ObjectUtil.isNull(user.getDeptId()) || StringUtils.isNotBlank(user.getDeptName())) {
            return;
        }
        SysDeptVo dept = sysDeptMapper.selectVoById(user.getDeptId());
