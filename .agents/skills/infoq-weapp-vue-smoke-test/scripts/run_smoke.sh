...
WEAPP_DIR="${REPO_ROOT}/infoq-scaffold-frontend-weapp-vue"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[weapp-vue-smoke] Unknown option: $1" >&2
      usage
      exit 1
      ;;
  esac
...
case "${SUITE}" in
  smoke|core|full)
    ;;
  *)
    echo "[weapp-vue-smoke] Invalid suite: ${SUITE}. Supported: smoke|core|full" >&2
    exit 1
    ;;
esac

if [[ ! -d "${WEAPP_DIR}" ]]; then
  echo "[weapp-vue-smoke] Workspace not found: ${WEAPP_DIR}" >&2
  exit 1
fi

if ! command -v node >/dev/null 2>&1; then
  echo "[weapp-vue-smoke] node is required but not found in PATH" >&2
  exit 1
fi
...
if command -v pnpm >/dev/null 2>&1; then
  ...
  echo "[weapp-vue-smoke] pnpm/npm is required but not found in PATH" >&2
  exit 1
fi

if ! command -v curl >/dev/null 2>&1; then
  echo "[weapp-vue-smoke] curl is required but not found in PATH" >&2
  exit 1
fi

if [[ "${LOGIN_HOME_ONLY}" -eq 1 ]]; then
  SUITE="smoke"
  echo "[weapp-vue-smoke] login-home-only mode enabled; force suite=smoke."
fi
...
if [[ "${BUILD_FIRST}" -eq 1 ]]; then
  echo "[weapp-vue-smoke] Building dist with ${PKG} run build:weapp:dev ..."
  run_script "build:weapp:dev"
fi

echo "[weapp-vue-smoke] Launching suite=${SUITE}, autoLogin=${AUTO_LOGIN}, keepExistingSession=${KEEP_EXISTING_SESSION}, urlCheck=${URL_CHECK}"
...
verify_backend_health() {
  ...
  if ! curl -fsS --max-time 5 "${probe_endpoint}" >"${probe_output}"; then
    echo "[weapp-vue-smoke] Backend health probe failed: ${probe_endpoint}" >&2
    echo "[weapp-vue-smoke] Make sure backend is running and reachable before weapp smoke." >&2
    return 1
  fi

  if grep -Fq '"captchaEnabled":true' "${probe_output}"; then
    echo "[weapp-vue-smoke] Backend health probe returned captchaEnabled=true at ${probe_endpoint}" >&2
    echo "[weapp-vue-smoke] For smoke login, start backend with --captcha.enable=false." >&2
    return 1
  fi

  echo "[weapp-vue-smoke] Backend health probe passed: ${probe_endpoint}"
}
...
if grep -Fq "[object Object]" "${RUN_LOG_FILE}"; then
  echo "[weapp-vue-smoke] Detected \"[object Object]\" in runner log, which violates mini-program error-message contract." >&2
  echo "[weapp-vue-smoke] Please normalize request failures in src/api/request.ts (errMsg/message/msg extraction + fallback)." >&2
  echo "[weapp-vue-smoke] Runner log: ${RUN_LOG_FILE}" >&2
  exit 1
fi
...
if [[ "${LOGIN_HOME_ONLY}" -eq 1 ]]; then
  ...
  if [[ "${home_route_pass}" -eq 1 && "${auth_injected}" -eq 1 ]]; then
    if [[ "${RUN_EXIT}" -eq 0 ]]; then
      echo "[weapp-vue-smoke] login-home-only passed."
      echo "[weapp-vue-smoke] Runner log: ${RUN_LOG_FILE}"
      exit 0
    fi
    if [[ "${known_login_redirect_mismatch}" -eq 1 ]]; then
      echo "[weapp-vue-smoke] login-home-only passed with known mismatch: authenticated /pages/login/index redirects to /pages/home/index."
      echo "[weapp-vue-smoke] Runner log: ${RUN_LOG_FILE}"
      exit 0
    fi
  fi

  echo "[weapp-vue-smoke] login-home-only failed." >&2
  echo "[weapp-vue-smoke] Runner log: ${RUN_LOG_FILE}" >&2
  exit 1
fi

if [[ "${RUN_EXIT}" -ne 0 ]]; then
  echo "[weapp-vue-smoke] Smoke suite failed. Runner log: ${RUN_LOG_FILE}" >&2
  exit "${RUN_EXIT}"
fi

echo "[weapp-vue-smoke] Smoke suite passed."
echo "[weapp-vue-smoke] Runner log: ${RUN_LOG_FILE}"
