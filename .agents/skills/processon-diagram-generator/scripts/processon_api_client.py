...
DSL_EDIT_URL = "https://smart.processon.com/render-dsl"
...
def build_ssl_context():
    no_verify = os.environ.get("PROCESSON_SSL_NO_VERIFY", "").strip().lower()
    if no_verify in {"1", "true", "yes", "on"}:
...
def generate_diagram(prompt, title=None, stream_style=None, output_mode=None, auto_render=True):
    ...
    def open_json_request(url, headers, payload, timeout=180):
        ...
        return urllib.request.urlopen(request, timeout=timeout, context=build_ssl_context())
